            ...
            <div class="tab-pane {{ $language->code === auth()->user()->language_code ? 'active' : '' }}" id="{{ $language->code }}-id" role="tabpanel" aria-labelledby="{{ $language->code }}">
             <div class="col-12">
              <fieldset class="form-group">
               ...
               <textarea {{ $language->code === auth()->user()->language_code ? 'required' : '' }} class="form-control" id="basicTextarea" rows="5" placeholder="Translations ({{ $language->name }})" name="translations[{{ $key }}][translation]">{{ old('translation') }}</textarea>
              </fieldset>
             </div>
            </div>
           @endforeach
