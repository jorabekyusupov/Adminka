         ...
         <div class="tab-pane {{ $language->code === auth()->user()->language_code ? 'active' : '' }}" id="{{ $language->code }}-id" role="tabpanel" aria-labelledby="{{ $language->code }}">
          <div class="col-12">
           <fieldset class="form-group">
            ...
            <input {{ $language->code === auth()->user()->language_code ? 'required' : '' }} type="text" class="form-control mb-1" name="translations[{{ $key }}][title]" placeholder="Category Name ({{ $language->code }})">
           </fieldset>
          </div>
         </div>
        @endforeach
